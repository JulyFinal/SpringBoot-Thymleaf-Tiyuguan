...
Varea._m = {
    ...
    addClass:function (c, node) {
        if(!node)return;
        node.className = Varea._m.hasClass(c,node) ? node.className : node.className + ' ' + c ;
    },

    /* 移除样式名 */
    removeClass:function (c, node) {
        var reg = new RegExp("(^|\\s+)" + c + "(\\s+|$)", "g");
        if(!Varea._m.hasClass(c,node))return;
...
function dataAreaHandle() {
	Varea.allArea = allArea;
    var areas = Varea.allArea, match, letter,
        regEx = Varea.regEx,
        reg2 = /^[a-h]$/i, reg3 = /^[i-p]$/i, reg4 = /^[q-z]$/i;
		Varea.oCode = {all:{},ABCDEFGH:{}, IJKLMNOP:{}, QRSTUVWXYZ:{}};
        Varea.oArea = {all:{},ABCDEFGH:{}, IJKLMNOP:{}, QRSTUVWXYZ:{}};
        for (var i = 0, n = areas.length; i < n; i++) {
            match = regEx.exec(areas[i]);
            if(!Varea.oArea.all['all']) Varea.oArea.all['all'] = [];
                Varea.oArea.all['all'].push(match[1]);
			if(!Varea.oCode.all['all']) Varea.oCode.all['all'] = [];
				Varea.oCode.all['all'].push(match[4]);
        }
};
//校验用户手动输入的区县名称,如果没有使用鼠标点击,会造成code为空或不正确,进而改成输入正确的区县名称后自动对隐藏域code进行赋值
//name:输入的区县名称   hiddenId:隐藏域ID
function areaValidateFn(name,hiddenId){
	var pageCode=$("#"+hiddenId).val();//隐藏域开市隐藏code
	if(name==null||name=="") return false;
	var validateCode = areaValidate[name];//通过区县名称获取正确区县code
...
Varea.AreaSelector = function () {
    this.initialize.apply(this, arguments);
};

Varea.AreaSelector.prototype = {

    constructor:Varea.AreaSelector,

    /* 初始化 */

    initialize :function (options) {
    	var input = options.input;
    	this.input = Varea._m.$('#'+ input);
    	this.inputEvent();
    ...
    createWarp:function(){
        var inputPos = Varea._m.getPos(this.input);
        var div = this.rootDiv = document.createElement('div');
        var that = this;

        // 设置DIV阻止冒泡
        Varea._m.on(this.rootDiv,'click',function(event){
            Varea._m.stopPropagation(event);
        });

        // 设置点击文档隐藏弹出的区县选择框
        Varea._m.on(document, 'click', function (event) {
            event = Varea._m.getEvent(event);
            var target = Varea._m.getTarget(event);
            if(target == that.input) return false;
            //console.log(target.className);
            if (that.AreaBox)Varea._m.addClass('hide', that.AreaBox);
            if (that.ul)Varea._m.addClass('hide', that.ul);
            if(that.myIframe)Varea._m.addClass('hide',that.myIframe);
        });
        ...
        childdiv.id = 'areaBox';
//        childdiv.innerHTML = Varea._template.join('');
        var allArea = this.allArea =  document.createElement('div');
        allArea.className = 'allArea';
        childdiv.appendChild(allArea);
    ...
    createAllArea:function(){
        var odiv,odl,odt,odd,odda=[],str,key,ckey,sortKey,regEx = Varea.regEx,
            oArea = Varea.oArea;
			oCode = Varea.oCode;
        for(key in oArea){
            ...
            // 移除热门区县的隐藏CSS
            Varea._m.removeClass('hide',this.all);
            this.allArea.appendChild(odiv);
        }
    ...
    tabChange:function(){
        var lis = Varea._m.$('li',this.AreaBox);
        var divs = Varea._m.$('div',this.hotArea);
        var that = this;
        for(var i=0,n=lis.length;i<n;i++){
            lis[i].index = i;
            lis[i].onclick = function(){
                for(var j=0;j<n;j++){
                    Varea._m.removeClass('on',lis[j]);
                    Varea._m.addClass('hide',divs[j]);
                }
                Varea._m.addClass('on',this);
                Varea._m.removeClass('hide',divs[this.index]);
    ...
    linkEvent:function(){
        var links = Varea._m.$('a',this.allArea);
        var that = this;
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                Varea._m.addClass('hide',that.AreaBox);
                /* 点击区县名的时候隐藏myIframe */
                Varea._m.addClass('hide',that.myIframe);
    ...
    inputEvent:function(){
        var that = this;
        Varea._m.on(this.input,'click',function(event){
            event = event || window.event;
            if(!that.AreaBox){
                that.createWarp();
            }else if(!!that.AreaBox && Varea._m.hasClass('hide',that.AreaBox)){
                // slideul 不存在或者 slideul存在但是是隐藏的时候 两者不能共存
                if(!that.ul || (that.ul && Varea._m.hasClass('hide',that.ul))){
                    Varea._m.removeClass('hide',that.AreaBox);

                    /* IE6 移除iframe 的hide 样式 */
                    //alert('click');
                    Varea._m.removeClass('hide',that.myIframe);
                    that.changeIframe();
                }
            }
        });
        Varea._m.on(this.input,'focus',function(){
            that.input.select();
            if(that.input.value == '支持中文输入') that.input.value = '';
        });
        Varea._m.on(this.input,'blur',function(){
            if(that.input.value == '')
            	document.getElementById(that.input.id).style.color='#aaa';
            else
            	document.getElementById(that.input.id).style.color='#333';
        });
        Varea._m.on(this.input,'keyup',function(event){
            event = event || window.event;
            ...
            // 下拉菜单显示的时候捕捉按键事件
            if(that.ul && !Varea._m.hasClass('hide',that.ul) && !that.isEmpty){
    ...
    createUl:function () {
        ...
        var str;
        var value = Varea._m.trim(this.input.value);
        // 当value不等于空的时候执行
        if (value !== '') {
            var reg = new RegExp("^" + value + "|\\|" + value, 'gi');
            var searchResult = [];
            for (var i = 0, n = Varea.allArea.length; i < n; i++) {
                if (reg.test(Varea.allArea[i])) {
                    var match = Varea.regEx.exec(Varea.allArea[i]);
                    if (searchResult.length !== 0) {
                        str = '<li code="'+ match[4] +'"><b class="areaname">' + match[1] + '</b></li>';
                    } else {
                        str = '<li class="on" code="'+ match[4] +'"><b class="areaname">' + match[1] + '</li>';
                    }
            ...
            if (!this.ul) {
                ...
                // 记录按键次数，方向键
                this.count = 0;
            } else if (this.ul && Varea._m.hasClass('hide', this.ul)) {
                this.count = 0;
                Varea._m.removeClass('hide', this.ul);
            }
            ...
            Varea._m.addClass('hide',this.ul);
            Varea._m.removeClass('hide',this.AreaBox);

            Varea._m.removeClass('hide',this.myIframe);
    ...
    KeyboardEvent:function(event,keycode){
        ...
        switch(keycode){
            case 40: //向下箭头↓
                ...
                for(var i=0;i<len;i++){
                    Varea._m.removeClass('on',lis[i]);
                }
                Varea._m.addClass('on',lis[this.count]);
                break;
            case 38: //向上箭头↑
                this.count--;
                ...
                break;
            case 13: // enter键
                this.input.value = Varea.regExChiese.exec(lis[this.count].innerHTML)[0];
				this.hinput.value = lis[this.count].getAttribute('code');
                Varea._m.addClass('hide',this.ul);
                Varea._m.addClass('hide',this.ul);
                /* IE6 */
                Varea._m.addClass('hide',this.myIframe);
                break;
            default:
                break;
        }
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
			...
			(function(data){
				 Varea._m.on(data,'click',function(event){
		                event = Varea._m.getEvent(event);
		                var target = Varea._m.getTarget(event);
		                that.input.value = Varea.regExChiese.exec(target.innerHTML)[0];
						that.hinput.value = data.getAttribute('code');
		                Varea._m.addClass('hide',that.ul);
		                /* IE6 下拉菜单点击事件 */
		                Varea._m.addClass('hide',that.myIframe);
		            });
			})(lis[i]);
			
            Varea._m.on(lis[i],'mouseover',function(event){
                event = Varea._m.getEvent(event);
                var target = Varea._m.getTarget(event);
                Varea._m.addClass('on',target);
            });
            Varea._m.on(lis[i],'mouseout',function(event){
                event = Varea._m.getEvent(event);
                var target = Varea._m.getTarget(event);
                Varea._m.removeClass('on',target);
            })
        }
    }
};