...
var allAreaStart=null;//存放区县信息的变量
/* *
 * 静态方法集
 * @name _m
 * */
VareaStart._m = {
    ...
    addClass:function (c, node) {
        if(!node)return;
        node.className = VareaStart._m.hasClass(c,node) ? node.className : node.className + ' ' + c ;
    },

    /* 移除样式名 */
    removeClass:function (c, node) {
        var reg = new RegExp("(^|\\s+)" + c + "(\\s+|$)", "g");
        if(!VareaStart._m.hasClass(c,node))return;
...
function dataAreaStartHandle() {
	VareaStart.allAreaStart = allAreaStart;
    var areas = VareaStart.allAreaStart, match, letter,
        regEx = VareaStart.regEx,
        reg2 = /^[a-h]$/i, reg3 = /^[i-p]$/i, reg4 = /^[q-z]$/i;
		VareaStart.oCode = {all:{},ABCDEFGH:{}, IJKLMNOP:{}, QRSTUVWXYZ:{}};
        VareaStart.oArea = {all:{},ABCDEFGH:{}, IJKLMNOP:{}, QRSTUVWXYZ:{}};
        for (var i = 0, n = areas.length; i < n; i++) {
            match = regEx.exec(areas[i]);
            if(!VareaStart.oArea.all['all']) VareaStart.oArea.all['all'] = [];
                VareaStart.oArea.all['all'].push(match[1]);
			if(!VareaStart.oCode.all['all']) VareaStart.oCode.all['all'] = [];
				VareaStart.oCode.all['all'].push(match[4]);
        }
};
//校验用户手动输入的区县名称,如果没有使用鼠标点击,会造成code为空或不正确,进而改成输入正确的区县名称后自动对隐藏域code进行赋值
//name:输入的区县名称   hiddenId:隐藏域ID
function areaStartValidateFn(pno,name,hiddenId){
...
VareaStart.AreaSelector = function () {
    this.initialize.apply(this, arguments);
};

VareaStart.AreaSelector.prototype = {

    constructor:VareaStart.AreaSelector,

    /* 初始化 */

    initialize :function (options) {
    	var input = options.input;
    	this.input = VareaStart._m.$('#'+ input);
    	this.inputEvent();
    ...
    createWarp:function(){
        var inputPos = VareaStart._m.getPos(this.input);
        var div = this.rootDiv = document.createElement('div');
        var that = this;

        // 设置DIV阻止冒泡
        VareaStart._m.on(this.rootDiv,'click',function(event){
            VareaStart._m.stopPropagation(event);
        });

        // 设置点击文档隐藏弹出的区县选择框
        VareaStart._m.on(document, 'click', function (event) {
            event = VareaStart._m.getEvent(event);
            var target = VareaStart._m.getTarget(event);
            if(target == that.input) return false;
            //console.log(target.className);
            if (that.AreaBox)VareaStart._m.addClass('hide', that.AreaBox);
            if (that.ul)VareaStart._m.addClass('hide', that.ul);
            if(that.myIframe)VareaStart._m.addClass('hide',that.myIframe);
        });
        ...
        childdiv.id = 'areaBox';
//        childdiv.innerHTML = VareaStart._template.join('');
        var allAreaStart = this.allAreaStart =  document.createElement('div');
        allAreaStart.className = 'allAreaStart';
        childdiv.appendChild(allAreaStart);
        div.appendChild(childdiv);
        this.createallAreaStart();
    },
    ...
    createallAreaStart:function(){
        var odiv,odl,odt,odd,odda=[],str,key,ckey,sortKey,regEx = VareaStart.regEx,
            oArea = VareaStart.oArea;
			oCode = VareaStart.oCode;
        for(key in oArea){
            ...
            // 移除热门区县的隐藏CSS
            VareaStart._m.removeClass('hide',this.all);
            this.allAreaStart.appendChild(odiv);
        }
    ...
    tabChange:function(){
        var lis = VareaStart._m.$('li',this.AreaBox);
        var divs = VareaStart._m.$('div',this.hotArea);
        var that = this;
        for(var i=0,n=lis.length;i<n;i++){
            lis[i].index = i;
            lis[i].onclick = function(){
                for(var j=0;j<n;j++){
                    VareaStart._m.removeClass('on',lis[j]);
                    VareaStart._m.addClass('hide',divs[j]);
                }
                VareaStart._m.addClass('on',this);
                VareaStart._m.removeClass('hide',divs[this.index]);
    ...
    linkEvent:function(){
        var links = VareaStart._m.$('a',this.allAreaStart);
        var that = this;
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                VareaStart._m.addClass('hide',that.AreaBox);
                /* 点击区县名的时候隐藏myIframe */
                VareaStart._m.addClass('hide',that.myIframe);
    ...
    inputEvent:function(){
        var that = this;
        VareaStart._m.on(this.input,'click',function(event){
            event = event || window.event;
            if(!that.AreaBox){
                that.createWarp();
            }else if(!!that.AreaBox && VareaStart._m.hasClass('hide',that.AreaBox)){
                // slideul 不存在或者 slideul存在但是是隐藏的时候 两者不能共存
                if(!that.ul || (that.ul && VareaStart._m.hasClass('hide',that.ul))){
                    VareaStart._m.removeClass('hide',that.AreaBox);

                    /* IE6 移除iframe 的hide 样式 */
                    //alert('click');
                    VareaStart._m.removeClass('hide',that.myIframe);
                    that.changeIframe();
                }
            }
        });
        VareaStart._m.on(this.input,'focus',function(){
            that.input.select();
            if(that.input.value == '支持中文输入') that.input.value = '';
        });
        VareaStart._m.on(this.input,'blur',function(){
            if(that.input.value == '')
            	document.getElementById(that.input.id).style.color='#aaa';
            else
            	document.getElementById(that.input.id).style.color='#333';
        });
        VareaStart._m.on(this.input,'keyup',function(event){
            event = event || window.event;
            ...
            // 下拉菜单显示的时候捕捉按键事件
            if(that.ul && !VareaStart._m.hasClass('hide',that.ul) && !that.isEmpty){
    ...
    createUl:function () {
        ...
        var str;
        var value = VareaStart._m.trim(this.input.value);
        // 当value不等于空的时候执行
        if (value !== '') {
            var reg = new RegExp("^" + value + "|\\|" + value, 'gi');
            var searchResult = [];
            for (var i = 0, n = VareaStart.allAreaStart.length; i < n; i++) {
                if (reg.test(VareaStart.allAreaStart[i])) {
                    var match = VareaStart.regEx.exec(VareaStart.allAreaStart[i]);
            ...
            if (!this.ul) {
                ...
                // 记录按键次数，方向键
                this.count = 0;
            } else if (this.ul && VareaStart._m.hasClass('hide', this.ul)) {
                this.count = 0;
                VareaStart._m.removeClass('hide', this.ul);
            }
            ...
            VareaStart._m.addClass('hide',this.ul);
            VareaStart._m.removeClass('hide',this.AreaBox);

            VareaStart._m.removeClass('hide',this.myIframe);
    ...
    KeyboardEvent:function(event,keycode){
        var lis = VareaStart._m.$('li',this.ul);
        var len = lis.length;
        switch(keycode){
            case 40: //向下箭头↓
                this.count++;
                ...
                for(var i=0;i<len;i++){
                    VareaStart._m.removeClass('on',lis[i]);
                }
                VareaStart._m.addClass('on',lis[this.count]);
                break;
            case 38: //向上箭头↑
                this.count--;
                ...
                break;
            case 13: // enter键
                this.input.value = VareaStart.regExChiese.exec(lis[this.count].innerHTML)[0];
				this.hinput.value = lis[this.count].getAttribute('code');
                VareaStart._m.addClass('hide',this.ul);
                VareaStart._m.addClass('hide',this.ul);
                /* IE6 */
                VareaStart._m.addClass('hide',this.myIframe);
                break;
            default:
                break;
        }
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
			...
			(function(data){
				 VareaStart._m.on(data,'click',function(event){
		                event = VareaStart._m.getEvent(event);
		                var target = VareaStart._m.getTarget(event);
		                that.input.value = VareaStart.regExChiese.exec(target.innerHTML)[0];
						that.hinput.value = data.getAttribute('code');
		                VareaStart._m.addClass('hide',that.ul);
		                /* IE6 下拉菜单点击事件 */
		                VareaStart._m.addClass('hide',that.myIframe);
		            });
			})(lis[i]);
			
            VareaStart._m.on(lis[i],'mouseover',function(event){
                event = VareaStart._m.getEvent(event);
                var target = VareaStart._m.getTarget(event);
                VareaStart._m.addClass('on',target);
            });
            VareaStart._m.on(lis[i],'mouseout',function(event){
                event = VareaStart._m.getEvent(event);
                var target = VareaStart._m.getTarget(event);
                VareaStart._m.removeClass('on',target);
            })
        }
    }
};